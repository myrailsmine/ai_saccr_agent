...
class SACCRApplication:
        ...
        with st.expander("Add New Trade", expanded=True):
            ...
            with trade_form:
                ...
                with col1:
                    ...
                    trade_type = st.selectbox("Trade Type", ["Swap", "Forward", "Option", "Swaption"])
    ...
    def _save_portfolio(self):
        ...
        if st.button("💾 Save") and portfolio_name:
            try:
                self.db_manager.save_portfolio(portfolio_name, st.session_state.current_portfolio)
    ...
    def _render_calculation_settings(self):
        ...
        alpha_unmargined = st.number_input(
            "Alpha (Unmargined)", 
            value=current_config.get('alpha_unmargined', 1.4),
            min_value=0.1, max_value=5.0, step=0.1
        )
        
        alpha_margined = st.number_input(
            "Alpha (Margined)", 
            value=current_config.get('alpha_margined', 1.4),
            min_value=0.1, max_value=5.0, step=0.1
        )
        
        if st.button("💾 Save Calculation Settings"):
            self.config_manager.update_calculation_config({
                'alpha_unmargined': alpha_unmargined,
                'alpha_margined': alpha_margined
            })
